...
const GamesList = () => {
  ...
  const createGameHandler = async () => {
    if (userName) {
      ...
      const response = await fetch('http://localhost:8080/games/new', requestOptions);
      const data = await response.json();
  ...
  const joinGame = async (gameId, index = gameIndex) => {
    if (userName && gameId) {
      ...
      const response = await fetch(`http://localhost:8080/players/${userName}/join/${gameId}`, requestOptions);
      const data = await response.json();
